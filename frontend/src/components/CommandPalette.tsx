      ...
      <div className="w-full max-w-xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="relative border-b border-slate-800 p-4 flex items-center gap-3">
          ...
          <input
            ...
            autoFocus
          />
          <button onClick={onClose} aria-label="关闭搜索" className="p-1 text-slate-400 hover:text-slate-200">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="max-h-80 overflow-y-auto p-3 space-y-2 text-xs">
          ...
          <button
            ...
            className="w-full flex items-center justify-between p-2.5 rounded-xl bg-slate-800/40 hover:bg-slate-800 text-slate-200 transition-all text-left"
          >
          ...
          <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 px-2 py-1 pt-2">
            热门美股与加拿大标的 (Stocks)
          </div>
          ...
          {filteredTickers.map((t) => (
            <button
              ...
              className="w-full flex items-center justify-between p-2.5 rounded-xl bg-slate-800/20 hover:bg-slate-800/80 text-slate-200 transition-all text-left"
            >
              <div className="flex items-center gap-2.5">
                <Sparkles className="w-4 h-4 text-emerald-400" />
                <div>
                  <span className="font-bold text-slate-100 mr-2">{t.symbol}</span>
          ...
          {query.trim() && !filteredTickers.some((t) => t.symbol.toLowerCase() === query.trim().toLowerCase()) && (
            <button
              ...
              className="w-full p-2.5 rounded-xl bg-emerald-950/40 border border-emerald-500/40 text-emerald-300 font-medium hover:bg-emerald-900/50 transition-all text-left flex items-center gap-2"
            >
        ...
        <div className="p-3 bg-slate-950 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400">
          <span>提示：按 <kbd className="px-1.5 py-0.5 bg-slate-800 rounded border border-slate-700 font-mono">Esc</kbd> 退出</span>
          <span className="text-emerald-400">Ctrl + K 随时唤起</span>
        </div>
      </div>
    </div>
  );
