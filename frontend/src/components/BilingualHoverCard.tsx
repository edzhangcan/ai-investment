...
export const BilingualHoverCard: React.FC<BilingualHoverCardProps> = ({
  ...
  const popoverContent = (
    <div
      ...
      className="p-4 bg-white dark:bg-slate-900 border-2 border-amber-500/70 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.25)] dark:shadow-[0_25px_60px_rgba(0,0,0,0.95)] text-left pointer-events-auto transition-opacity duration-150 animate-fade-in"
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2 mb-3">
        ...
        <div className="flex items-center gap-1 text-[10px] bg-amber-50 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 px-2 py-0.5 rounded-full border border-amber-300 dark:border-amber-500/40 font-extrabold uppercase">
      ...
      {language !== 'zh' && (
        <div className="mb-2.5">
          <div className="text-[10px] font-bold tracking-wider uppercase text-amber-600 dark:text-amber-400/90 mb-0.5">🇺🇸 English Definition:</div>
          <p className="text-xs text-slate-800 dark:text-slate-100 font-medium leading-relaxed bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
            {defEn}
          </p>
        </div>
      )}
      ...
      {language !== 'en' && (
        <div className="mb-2.5">
          <div className="text-[10px] font-bold tracking-wider uppercase text-amber-600 dark:text-amber-400/90 mb-0.5">🇨🇳 中文解析：</div>
          <p className="text-xs text-slate-800 dark:text-slate-100 font-medium leading-relaxed bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
            {defZh}
          </p>
        </div>
      ...
      {((language !== 'zh' && analogyEn) || (language !== 'en' && analogyZh)) && (
        <div className="p-3 bg-amber-50/70 dark:bg-slate-950 rounded-xl border border-amber-300 dark:border-amber-500/40 text-xs text-amber-900 dark:text-amber-200 leading-relaxed shadow-sm">
          <div className="font-bold text-amber-700 dark:text-amber-300 text-xs mb-1 flex items-center gap-1">
            <span>{t.everydayAnalogyHeader}</span>
          </div>
          {language !== 'zh' && analogyEn && (
            <p className="mb-1 text-[11px] text-amber-950 dark:text-amber-100 font-normal">🇺🇸 {analogyEn}</p>
          )}
          {language !== 'en' && analogyZh && (
            <p className="text-[11px] text-amber-950 dark:text-amber-100 font-normal">🇨🇳 {analogyZh}</p>
          )}
        </div>
      )}
    </div>
    ...
    >
      <span
        className={`transition-all inline-flex items-center gap-1 ${
          isPlainTalk
            ? 'border-b-2 border-dashed border-amber-500 font-bold text-amber-700 dark:text-amber-300 hover:text-amber-900 dark:hover:text-amber-200 px-1.5 py-0.5 bg-amber-50 dark:bg-amber-950/40 rounded-lg shadow-sm'
            : 'hover:text-sky-600 dark:hover:text-emerald-300 border-b border-dashed border-slate-300 dark:border-slate-600'
        }`}
      >
        {displayTitle}
        <HelpCircle className={`w-3.5 h-3.5 inline shrink-0 ${isPlainTalk ? 'text-amber-500 animate-pulse' : 'text-slate-400'}`} />
      </span>
