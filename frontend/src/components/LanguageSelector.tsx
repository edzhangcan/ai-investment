  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        ...
        className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-emerald-500/50 rounded-xl text-xs font-semibold text-slate-200 flex items-center gap-2 transition-all cursor-pointer shadow-md"
        title="Switch Interface Language (切换系统语言)"
      >
        <Globe className="w-3.5 h-3.5 text-emerald-400" />
        <span className="flex items-center gap-1">
          <span>{currentOpt.flag}</span>
          <span className="font-bold text-slate-100">{currentOpt.badge}</span>
        </span>
        <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-slate-900/95 border border-slate-700/90 rounded-2xl shadow-2xl backdrop-blur-xl z-50 py-1.5 animate-fade-in">
